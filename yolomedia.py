...
# ========= YOLO实时矫正参数 =========
YOLO_CORRECTION_IOU_THRESHOLD = 0.4  # IoU閾值（從0.2提高到0.4，減少不必要的頻繁校正）
YOLO_CORRECTION_CONF_THRESHOLD = 0.20  # 置信度閾值（從0.15提高到0.20，降低誤檢率）
    ...
    try:
        while True:
            ...
            if MODE == "SEGMENT":
                ...
                if use_yoloe and yoloe_backend is not None and (FRAME_IDX % 2 == 0):
                    # 每 2 幀偵測一次（降低 GPU 負載，提升流暢度）；persist=True 維持目標 ID
                ...
                if old_gray is not None and p0 is not None and len(p0) >= 5:
                    ...
                    if p1 is not None and st is not None:
                        ...
                        if len(good_new) >= 5:
                            ...
                            if len(poly) >= 3:
                                # YOLOE 實時檢測和校正（每 2 幀，降低 GPU 負載提升流暢度）
                                latest_det_mask = None
                                if use_yoloe and yoloe_backend is not None and (track_frame_count % 2 == 0):
                                    # 添加调试信息
